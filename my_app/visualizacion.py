...
def visualizar(df_metrics, mejores_modelos, df_leaderboard_testset,df_feature_importance,df_test_pred):
    ...
    for i, modelo in enumerate(mejores_modelos):
        with tabs[i]:
            with st.expander(f"Métricas por semilla para el modelo: {modelo}", expanded=False):
            ...
            # --- Configuraciones ---
            with st.expander(f"Hiperparámetros del modelo: {modelo}", expanded=False):
            ...
            with st.expander(f"Inportancia de las variables por: {modelo}", expanded=False):
                # Filtrar filas
            ...
            with st.expander(f"Análisis del Roc_auc por Fold para el: {modelo}", expanded=False):
        ...
        ##   Errores de clasificación por modelo
            with st.expander(f"Errores de clasificación por modelo: {modelo}", expanded=False):
